...
void Battle::splash( std::ostream & out ) const {
  ...
  out << "  _______      ___      .___  ___.  _______ " <<  std::endl;
  out << " /  _____|    /   \\     |   \\/   | |   ____|" <<  std::endl;
  out << "|  |  __     /  ^  \\    |  \\  /  | |  |_" <<  std::endl;
  out << "|  | |_ |   /  /_\\  \\   |  |\\/|  | |   __|" <<  std::endl;
  out << "|  |__| |  /  _____  \\  |  |  |  | |  |____" <<  std::endl;
  out << " \\______| /__/     \\__\\ |__|  |__| |_______|" <<  std::endl;
  out << "  ______    _______" <<  std::endl;
  out << " /  __  \\  |   ____|" <<  std::endl;
  out << "|  |  |  | |  |__" <<  std::endl;
  out << "|  |  |  | |   __|" <<  std::endl;
  out << "|  `--'  | |  |" <<  std::endl;
  out << " \\______/  |__|" <<  std::endl;
  out << "  ______   .______          ___       _______   ______   .__   __.      _______." <<  std::endl;
  out << " /  __  \\  |   _  \\        /   \\     /  _____| /  __  \\  |  \\ |  |     /       |" <<  std::endl;
  out << "|  |  |  | |  |_)  |      /  ^  \\   |  |  __  |  |  |  | |   \\|  |    |   (----`" <<  std::endl;
  
  out << "|  |  |  | |      /      /  /_\\  \\  |  | |_ | |  |  |  | |  . `  |     \\   \\" <<  std::endl;
  out << "|  `--'  | |  |\\  \\----./  _____  \\ |  |__| | |  `--'  | |  |   | .----)   |" <<  std::endl;
...
bool Battle::performUserActions( std::istream & in ) {
  ...
  for ( coll_type::iterator it = pcs_.begin(); it != pcs_.end(); ++it ) {
    ...
    std::cout << std::endl << "Action for " << (*it)->name() << ": " << std::endl;
    QuickAction qa;
    std::string line;
    in >> line; 
    //std::getline( in, line );
